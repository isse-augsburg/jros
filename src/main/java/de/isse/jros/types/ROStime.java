 ...
 * A ROS time type
 */
public class ROStime extends ROSType<int[]> {
	/**
	 * A ROS time type
	 */
	public static final ROStime TYPE = new ROStime();

	/**
	 * ROStime serves as singleton
	 */
	protected ROStime() {
	}
	...
	public int[] read(byte[] data, int position) {
		return new int[] { readSec(data, position), readNsec(data, position) };
	}
